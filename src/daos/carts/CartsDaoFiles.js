...
const productsFile = new ContainerFiles("../../../../DB/products.json");

class CartsDaoFiles extends ContainerFiles {
  constructor() {
    super();
  ...
  async createCart() {
    try {
      ...
      await this.fs.writeFile(this.path, JSON.stringify(this.carts), "utf-8");
  ...
  async addToCartById(id_cart, id_prod) {
    try {
      this.carts = await this.readCarts();
      const product = await super.readOne(id_prod);
  ...
  async deleteCartProductById(id_cart, id_prod) {
    try {
      ...
      const filteredProducts = prodsInCart.filter(
